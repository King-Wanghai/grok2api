...
class AuthTokenManager:
    ...
    def set_token(self, token):
        ...
        self.token_model_map = {model: [{"token": token, "RequestCount": 0, "AddedTime": int(time.time() * 1000), "StartCallTime": None}] for model in models}
        sso = token.split("sso=")[1].split(";")[0]
    ...
    def reduce_token_request_count(self, model_id, count):
        try:
            ...
            if normalized_model not in self.token_model_map or not self.token_model_map[normalized_model]:
                logger.error(f"模型 {normalized_model} 不存在或无可用token", "TokenManager")
                return False
            ...
            if sso in self.token_status_map and normalized_model in self.token_status_map[sso]:
                self.token_status_map[sso][normalized_model]["totalRequestCount"] = max(0, self.token_status_map[sso][normalized_model]["totalRequestCount"] - reduction)
            return True
        except Exception as error:
            logger.error(f"重置token请求次数失败: {str(error)}", "TokenManager")
            return False
    ...
    def remove_token_from_model(self, model_id, token):
        ...
        if token_index != -1:
            ...
            self.expired_tokens.add((removed_token_entry["token"], normalized_model, int(time.time() * 1000)))
            logger.info(f"模型{model_id}的令牌已失效，已移除: {token}", "TokenManager")
            return True
        logger.error(f"未找到模型 {normalized_model} 中的token: {token}", "TokenManager")
        return False
        ...
        def reset_expired_tokens():
            while True:
                ...
                for token_info in self.expired_tokens:
                    ...
                    if now - expired_time >= self.model_config[model]["ExpirationTime"]:
                ...
                for model in self.model_config.keys():
                    if model in self.token_model_map:
                        for token_entry in self.token_model_map[model]:
                            if token_entry.get("StartCallTime") and now - token_entry["StartCallTime"] >= self.model_config[model]["ExpirationTime"]:
                                sso = token_entry["token"].split("sso=")[1].split(";")[0]
                                if sso in self.token_status_map and model in self.token_status_map[sso]:
                                    self.token_status_map[sso][model]["isValid"] = True
                                    self.token_status_map[sso][model]["invalidatedTime"] = None
                ...
                time.sleep(3600)
        timer_thread = threading.Thread(target=reset_expired_tokens)
        timer_thread.daemon = True
        timer_thread.start()

    def get_all_tokens(self):
        return list({entry["token"] for model_tokens in self.token_model_map.values() for entry in model_tokens})
...
class Utils:
    ...
    def organize_search_results(search_results):
        ...
        formatted_results = [f"\r\n<details><summary>资料[{index}]: {result.get('title', '未知标题')}</summary>\r\n{result.get('preview', '无预览内容')}\r\n\n[Link]({result.get('url', '#')})\r\n</details>" for index, result in enumerate(results)]
...
class GrokApiClient:
    ...
    def get_image_type(self, base64_string):
        ...
        return {"mimeType": mime_type, "fileName": f"image.{mime_type.split('/')[1]}"}
    ...
    def upload_base64_file(self, message, model):
        try:
            ...
            cookie = f"{Utils.create_auth_headers(model, True)};{CONFIG['SERVER']['CF_CLEARANCE']}" if CONFIG['SERVER']['CF_CLEARANCE'] else Utils.create_auth_headers(model, True)
            response = curl_requests.post(
                "https://grok.com/rest/app-chat/upload-file",
                headers={**DEFAULT_HEADERS, "Cookie": cookie},
                json=upload_data,
                impersonate="chrome133a",
                **Utils.get_proxy_options()
            )
    ...
    def upload_base64_image(self, base64_data, url):
        try:
            ...
            upload_data = {"rpc": "uploadFile", "req": {"fileName": image_info["fileName"], "fileMimeType": image_info["mimeType"], "content": image_buffer}}
    ...
    def prepare_chat_request(self, request):
        if request["model"] in ['grok-2-imageGen', 'grok-3-imageGen'] and not CONFIG["API"]["PICGO_KEY"] and not CONFIG["API"]["TUMY_KEY"] and request.get("stream", False):
            raise ValueError("该模型流式输出需要配置PICGO或TUMY图床密钥!")
        todo_messages = [request["messages"][-1]] if request["model"] in ['grok-2-imageGen', 'grok-3-imageGen', 'grok-3-deepsearch'] else request["messages"]
        if todo_messages and todo_messages[-1]["role"] != 'user' and request["model"] in ['grok-2-imageGen', 'grok-3-imageGen', 'grok-3-deepsearch']:
        ...
        def process_content(content):
            if isinstance(content, list):
                ...
                return "[图片]" if content["type"] == 'image_url' else remove_think_tags(content["text"])
            ...
            if is_last_message and "content" in current:
                if isinstance(current["content"], list):
                    for item in current["content"]:
                        if item["type"] == 'image_url':
                            processed_image = self.upload_base64_image(item["image_url"]["url"], f"{CONFIG['API']['BASE_URL']}/api/rpc")
                            if processed_image:
                                file_attachments.append(processed_image)
                elif isinstance(current["content"], dict) and current["content"].get("type") == 'image_url':
                    processed_image = self.upload_base64_image(current["content"]["image_url"]["url"], f"{CONFIG['API']['BASE_URL']}/api/rpc")
                    if processed_image:
                        file_attachments.append(processed_image)
...
class MessageProcessor:
    ...
    def create_chat_response(message, model, is_stream=False):
        base_response = {"id": f"chatcmpl-{uuid.uuid4()}", "created": int(time.time()), "model": model}
        if is_stream:
            return {**base_response, "object": "chat.completion.chunk", "choices": [{"index": 0, "delta": {"content": message}}]}
        return {**base_response, "object": "chat.completion", "choices": [{"index": 0, "message": {"role": "assistant", "content": message}, "finish_reason": "stop"}], "usage": None}

def process_model_response(response, model):
    result = {"token": None, "imageUrl": None}
    if CONFIG["IS_IMG_GEN"] and response.get("cachedImageGenerationResponse") and not CONFIG["IS_IMG_GEN2"]:
        result["imageUrl"] = response["cachedImageGenerationResponse"]["imageUrl"]
        return result
    if model == 'grok-2' or model == 'grok-3':
        result["token"] = response.get("token")
    elif model in ['grok-2-search', 'grok-3-search'] and response.get("webSearchResults") and CONFIG["ISSHOW_SEARCH_RESULTS"]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not CONFIG["API"]["PICGO_KEY"] and not CONFIG["API"]["TUMY_KEY"]:
                    ...
                    return f"![image](data:{response.headers.get('content-type', 'image/jpeg')};base64,{base64_image})"
                logger.info("开始上传图床", "Server")
                files = {'source' if CONFIG["API"]["PICGO_KEY"] else 'file': ('image.jpg', image_buffer, 'image/jpeg')}
                headers = {"X-API-Key": CONFIG["API"]["PICGO_KEY"]} if CONFIG["API"]["PICGO_KEY"] else {"Accept": "application/json", 'Authorization': f"Bearer {CONFIG['API']['TUMY_KEY']}"}
                response_url = requests.post(
                    "https://www.picgo.net/api/1/upload" if CONFIG["API"]["PICGO_KEY"] else "https://tu.my/api/v1/upload",
                    files=files,
                    headers=headers
                )
                ...
                return f"![image]({result['image']['url'] if CONFIG['API']['PICGO_KEY'] else result['data']['links']['url']})"
            retry_count += 1
            ...
            if retry_count == max_retries:
                raise Exception(f"获取图片失败: {str(error)}")
            time.sleep(CONFIG["API"]["RETRY_TIME"] / 1000 * retry_count)

def handle_non_stream_response(response, model):
    try:
        logger.info("处理非流式响应", "Server")
        full_response = ""
        ...
        for chunk in response.iter_lines():
            ...
            try:
                ...
                logger.error(f"处理响应行出错: {str(e)}", "Server")
    ...
    def generate():
        ...
        for chunk in response.iter_lines():
            ...
            try:
                ...
                if result["imageUrl"]:
                    CONFIG["IS_IMG_GEN2"] = True
                    yield f"data: {json.dumps(MessageProcessor.create_chat_response(handle_image_response(result['imageUrl']), model, True))}\n\n"
            except json.JSONDecodeError:
                continue
            except Exception as e:
                logger.error(f"处理流式响应出错: {str(e)}", "Server")
        yield "data: [DONE]\n\n"
    return generate()
...
def initialization():
    ...
    logger.info(f"共加载: {len(token_manager.get_all_tokens())}个令牌", "Server")
    if CONFIG["API"]["PROXY"]:
...
def get_models():
    return jsonify({
        "object": "list",
        "data": [{"id": model, "object": "model", "created": int(time.time()), "owned_by": "grok"} for model in CONFIG["MODELS"].keys()]
    })
        ...
        while retry_count < CONFIG["RETRY"]["MAX_ATTEMPTS"]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    return Response(stream_with_context(handle_stream_response(response, model)), content_type='text/event-stream') if stream else jsonify(MessageProcessor.create_chat_response(handle_non_stream_response(response, model), model))
                elif response.status_code == 403:
                    response_status_code = 403
                    token_manager.reduce_token_request_count(model, 1)
                    if token_manager.get_token_count_for_model(model) == 0:
                        raise ValueError(f"{model} 次数已达上限，请切换模型或重试")
                    raise ValueError("IP暂时被封，请稍后重试或更换IP")
                elif response.status_code == 429:
                    response_status_code = 429
                    token_manager.reduce_token_request_count(model, 1)
                    if CONFIG["API"]["IS_CUSTOM_SSO"]:
                        raise ValueError(f"自定义SSO令牌{model}请求次数已失效")
                    token_manager.remove_token_from_model(model, CONFIG["API"]["SIGNATURE_COOKIE"])
                    if token_manager.get_token_count_for_model(model) == 0:
                        raise ValueError(f"{model} 次数已达上限，请切换模型或重试")
                else:
                    logger.error(f"异常状态码: {response.status_code}", "Server")
                    token_manager.remove_token_from_model(model, CONFIG["API"]["SIGNATURE_COOKIE"])
            except Exception as e:
