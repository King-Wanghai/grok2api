...
class Logger:
    def __init__(self, level="INFO", colorize=True, format=None):
        ...
        logger.add(sys.stderr, level=level, format=format,
                   colorize=colorize, backtrace=True, diagnose=True)
        self.logger = logger
    ...
    def error(self, message, source="API"):
        ...
        if isinstance(message, Exception):
            self.logger.bind(
                **caller_info).exception(f"[{source}] {str(message)}")
        else:
...
# 验证和更新CF_CLEARANCE的函数（占位，需要根据实际环境实现）


def validate_cf_clearance():
    if not CONFIG["SERVER"]["CF_CLEARANCE"]:
        logger.warning("CF_CLEARANCE 未设置或为空", "Server")
        return False
    test_response = curl_requests.get(
        "https://grok.com",
        headers={**DEFAULT_HEADERS,
                 "Cookie": f"cf_clearance={CONFIG['SERVER']['CF_CLEARANCE']}"},
        impersonate="chrome133a"
    )
    if test_response.status_code == 403:
        logger.error("CF_CLEARANCE 已失效", "Server")
        return False
    return True


def update_cf_clearance():
    # 这里需要实现通过浏览器或其他方式获取新的CF_CLEARANCE
    # 示例：使用selenium获取（需额外安装依赖）
    logger.warning("需要更新CF_CLEARANCE，请手动实现此逻辑", "Server")
    return CONFIG["SERVER"]["CF_CLEARANCE"]  # 返回现有值作为占位

# AuthTokenManager类保持不变，仅添加日志增强
...
class AuthTokenManager:
    ...
    def add_token(self, token):
        ...
        for model in self.model_config.keys():
            ...
            existing_token_entry = next(
                (entry for entry in self.token_model_map[model] if entry["token"] == token), None)
        ...
        logger.info(f"添加令牌: {token}", "TokenManager")

    def set_token(self, token):
        models = list(self.model_config.keys())
        self.token_model_map = {model: [{"token": token, "RequestCount": 0, "AddedTime": int(
            time.time() * 1000), "StartCallTime": None}] for model in models}
        sso = token.split("sso=")[1].split(";")[0]
        self.token_status_map[sso] = {model: {
            "isValid": True, "invalidatedTime": None, "totalRequestCount": 0} for model in models}
        logger.info(f"设置令牌: {token}", "TokenManager")
    ...
    def delete_token(self, token):
        try:
            ...
            for model in self.token_model_map:
                self.token_model_map[model] = [
                    entry for entry in self.token_model_map[model] if entry["token"] != token]
    ...
    def reduce_token_request_count(self, model_id, count):
        try:
            ...
            if not self.token_model_map[normalized_model]:
                logger.error(
                    f"模型 {normalized_model} 没有可用的token", "TokenManager")
                return False
    ...
    def get_next_token_for_model(self, model_id):
        ...
        if token_entry:
            ...
            if token_entry["RequestCount"] > self.model_config[normalized_model]["RequestFrequency"]:
                self.remove_token_from_model(
                    normalized_model, token_entry["token"])
                next_token_entry = self.token_model_map[normalized_model][
                    0] if self.token_model_map[normalized_model] else None
                return next_token_entry["token"] if next_token_entry else None
            sso = token_entry["token"].split("sso=")[1].split(";")[0]
            if sso in self.token_status_map and normalized_model in self.token_status_map[sso]:
                if token_entry["RequestCount"] == self.model_config[normalized_model]["RequestFrequency"]:
                    self.token_status_map[sso][normalized_model]["isValid"] = False
                    self.token_status_map[sso][normalized_model]["invalidatedTime"] = int(
                        time.time() * 1000)
    ...
    def remove_token_from_model(self, model_id, token):
        ...
        token_index = next((i for i, entry in enumerate(
            model_tokens) if entry["token"] == token), -1)
        if token_index != -1:
            removed_token_entry = model_tokens.pop(token_index)
            self.expired_tokens.add(
                (removed_token_entry["token"], normalized_model, int(time.time() * 1000)))
            if not self.token_reset_switch:
                self.start_token_reset_process()
                self.token_reset_switch = True
    ...
    def get_remaining_token_request_capacity(self):
        ...
        for model in self.model_config.keys():
            ...
            total_used_requests = sum(token_entry.get(
                "RequestCount", 0) for token_entry in model_tokens)
            remaining_capacity = (
                len(model_tokens) * model_request_frequency) - total_used_requests
...
class Utils:
    ...
    def get_proxy_options():
        ...
        if not proxy and CONFIG["API"]["PROXY_POOL"]:
            proxy = random.choice(CONFIG["API"]["PROXY_POOL"])
...
class GrokApiClient:
    ...
    def upload_base64_image(self, base64_data, url):
        try:
            ...
            upload_data = {
                "rpc": "uploadFile",
                "req": {"fileName": file_name, "fileMimeType": mime_type, "content": image_buffer}
            }
    ...
    def prepare_chat_request(self, request):
        if ((request["model"] == 'grok-2-imageGen' or request["model"] == 'grok-3-imageGen') and
                not CONFIG["API"]["PICGO_KEY"] and not CONFIG["API"]["TUMY_KEY"] and request.get("stream", False)):
            ...
            if isinstance(content, list):
                ...
                for item in content:
                    if item["type"] == 'image_url':
                        ...
                        text_content += (remove_think_tags(
                            item["text"]) if not text_content else '\n' + remove_think_tags(item["text"]))
                return text_content
            ...
            if text_content or (is_last_message and file_attachments):
                if role == last_role and text_content:
                    ...
                    messages = messages[:messages.rindex(
                        f"{role.upper()}: ")] + f"{role.upper()}: {last_content}\n"
                else:
...
# MessageProcessor类保持不变


class MessageProcessor:
    @staticmethod
    def create_chat_response(message, model, is_stream=False):
        base_response = {"id": f"chatcmpl-{uuid.uuid4()}",
...
def initialization():
    ...
    logger.info(
        f"成功加载令牌: {json.dumps(token_manager.get_all_tokens(), indent=2)}", "Server")
    logger.info(
        f"令牌加载完成，共加载: {len(token_manager.get_all_tokens())}个令牌", "Server")
    if CONFIG["API"]["PROXY"]:
        logger.info(f"代理已设置: {CONFIG['API']['PROXY']}", "Server")
    if CONFIG["API"]["PROXY_POOL"]:
        logger.info(f"代理池已设置: {CONFIG['API']['PROXY_POOL']}", "Server")
...
def chat_completions():
    ...
    try:
        ...
        while retry_count < CONFIG["RETRY"]["MAX_ATTEMPTS"]:
            ...
            # 验证并更新CF_CLEARANCE
            if not validate_cf_clearance():
                CONFIG["SERVER"]["CF_CLEARANCE"] = update_cf_clearance()
            ...
            try:
                ...
                logger.info(
                    f"请求头: {json.dumps(DEFAULT_HEADERS, indent=2)}, Cookie: {CONFIG['SERVER']['COOKIE']}", "Server")
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"403错误 - 请求头: {json.dumps(DEFAULT_HEADERS, indent=2)}, Cookie: {CONFIG['SERVER']['COOKIE']}, 响应: {response.text}", "Server")
                    if CONFIG["API"]["PROXY_POOL"]:
                        CONFIG["API"]["PROXY"] = random.choice(
                            CONFIG["API"]["PROXY_POOL"])
                        logger.info(
                            f"切换代理至: {CONFIG['API']['PROXY']}", "Server")
                    ...
                    logger.error(
                        f"未知错误状态码: {response.status_code}, 响应: {response.text}", "Server")
                    token_manager.remove_token_from_model(
                        model, CONFIG["API"]["SIGNATURE_COOKIE"])
                    continue
        ...
        if response_status_code == 403:
            raise ValueError('IP暂时被封无法破盾，请稍后重试或更换IP')
